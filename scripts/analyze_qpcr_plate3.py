#For version 2, add option of not using a control primer
#For version 3, add option of using a control primer but not using a control sample (i.e put WT on the same graph)
import pandas as pd
...
def main(arglist):
    ...
    parser.add_argument('datasheet',
                        help='excel sheet with Ct vals')
    parser.add_argument('template',
                        help='excel sheet with experiment setup')
    parser.add_argument('-ctrl_primer',
                        help='primer for normalization, optional')
    parser.add_argument('-drop_samples', nargs='+',
                        help='list of samples to drop', default=[])
    ...
    data_df = read_plate_data(args.datasheet)
    ...
    if args.ctrl_primer is not None:
        ctrl_df = well_df[well_df['primer'] == args.ctrl_primer].copy().groupby('sample').mean()
        ctrl_df.columns = ['ctrl_Cq']
    else:
        #create df of same structure but set the values to 0
        ctrl_df = well_df.copy().groupby('sample').mean()
        ctrl_df['Cq'] = 0
        ctrl_df.columns = ['ctrl_Cq']

    exp_df = well_df[well_df['primer'] != args.ctrl_primer].copy().groupby(['sample', 'primer']).mean()
    ...
    #adj ctrl Cq still needs to be 0 if there's no ctrl primer
    dCt_df['adj_ctrl_Cq'] = dCt_df['ctrl_Cq'] - (1/dCt_df['fraction']).apply(np.log2)
    #adj ctrl Cq still needs to be 0 if there's no ctrl primer
    if args.ctrl_primer is None:
         dCt_df['adj_ctrl_Cq'] = 0
    ...
    result_df = result_df[~result_df.index.get_level_values('sample').isin(args.drop_samples)].copy()
    return result_df
