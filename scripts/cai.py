...
class CodonAdaptationIndex:
    ...
    def set_cai_index(self, index, value_type="RSCU"):
        ...
        If value_type = RSCU, then convert these to the w values (0 - 1).
        Otherwise assume the values are w (RSCU normalized to max RSCU by AA)
        """
        if value_type == "RSCU":
            #normalize codon scores by max for that amino acid to create w (metric RSCUi/RSCUmax)
            codon_scores_norm = {}
            for aa in SynonymousCodons:
                if aa in ['MET', 'TRP', 'STOP']:
                    continue
                scores = []
                for codon in SynonymousCodons[aa]:
                    scores.append(index[codon])
                max_score = max(scores)
                for codon in SynonymousCodons[aa]:
                    codon_scores_norm[codon] = index[codon]/max_score
            self.index = codon_scores_norm
    ...
    def cai_for_gene(self, dna_sequence):
        ...
        skipped_codons = set(["ATG", "TGG", "TGA", "TAA", "TAG"])
